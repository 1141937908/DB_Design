...
public class BookTable extends JFrame implements ActionListener {
	...
	public BookTable() {
		...
		book = new Book();
		String[] paras = { "1" };
		book.queryBook("USE DB_Design;SELECT * FROM Book WHERE 1=?", paras);
		jTable2 = new JTable(book);
	...
	public void actionPerformed(ActionEvent e) {
		...
		/* 新增 */
			...
			book = new Book();
			String sql = "USE DB_Design;SELECT * FROM Book WHERE 1=?";
			String[] paras = { "1" };
			try {
				book.queryBook(sql, paras);
			} catch (Exception e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
			jTable2.setModel(book);
		}
		/* 删除 */
			...
			String BookId = (String) book.getValueAt(rowIndex, 0);
			System.out.println(BookId);
			String sql = "USE DB_Design;DELETE FROM Book WHERE Bno=?";
			String[] paras = { BookId };
			Book book = new Book();
			try {
				book.updateBook(sql, paras);
			} catch (Exception e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
			book = new Book();
			sql = "USE DB_Design;SELECT * FROM Book WHERE 1=?";
			String[] paras2 = { "1" };
			try {
				book.queryBook(sql, paras2);
			} catch (Exception e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
		...
		/* 修改 */
			...
			ModifyBookInfo modifyBookInfo = new ModifyBookInfo(this, "modify", true, book, rowIndex);
		...
		/* 查询 */
			...
			String Bno = this.jTextField.getText().trim();
			String sql = "USE DB_Design;SELECT * FROM Book WHERE Bno=?";
			String[] paras = { Bno };
			Book book1 = new Book();
			try {
				book1.queryBook(sql, paras);
			} catch (Exception e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
			jTable2.setModel(book1);
		}
		/* 刷新 */
