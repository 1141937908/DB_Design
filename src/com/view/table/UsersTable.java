...
import com.view.addition.AddUsersInfo;
import com.view.modify.ModifyBookInfo;
import com.view.modify.ModifyUsersInfo;
...
public class UsersTable extends JFrame implements ActionListener {
	...
	public UsersTable() {
		...
		users = new Users();
		String[] paras = { "1" };
		users.queryUsers("USE DB_Design;SELECT * FROM Users WHERE 1=?", paras);
		jTable2 = new JTable(users);
	...
	public void actionPerformed(ActionEvent e) {
		...
		/* 新增 */
			...
			AddUsersInfo addUsersInfo = new AddUsersInfo(this, "add", true);

			users = new Users();
			String sql = "USE DB_Design;SELECT * FROM Users WHERE 1=?";
			String[] paras = { "1" };
			try {
		...
		/* 删除 */
			...
			String UsersId = (String) users.getValueAt(rowIndex, 0);
			System.out.println(UsersId);
			String sql = "USE DB_Design;DELETE FROM Users WHERE Uno=?";
			String[] paras = { UsersId };
			Users users = new Users();
			try {
				users.updateUsers(sql, paras);
			} catch (Exception e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
			users = new Users();
			sql = "USE DB_Design;SELECT * FROM Users WHERE 1=?";
			String[] paras2 = { "1" };
			try {
				users.queryUsers(sql, paras2);
			} catch (Exception e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
		...
		/* 修改 */
			...
			ModifyUsersInfo modifyUsersInfo = new ModifyUsersInfo(this, "modify", true, users, rowIndex);

			Users users1 = new Users();
			String sql = "USE DB_Design;SELECT * FROM Users WHERE 1=?";
			String[] paras2 = { "1" };
			try {
				users1.queryUsers(sql, paras2);
			} catch (Exception e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
			jTable2.setModel(users1);

		}
		/* 查询 */
		else if (e.getSource() == jButton4) {
			String Uno = this.jTextField.getText().trim();
			String sql = "USE DB_Design;SELECT * FROM Users WHERE Uno=?";
			String[] paras = { Uno };
			users = new Users();
			try {
				users.queryUsers(sql, paras);
